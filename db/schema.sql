...
-- Service role only — relay uses SUPABASE_SERVICE_ROLE_KEY which bypasses RLS.
-- These policies block anon key access entirely.
CREATE POLICY "Service role only" ON messages FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service role only" ON memory FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service role only" ON logs FOR ALL USING (auth.role() = 'service_role');
