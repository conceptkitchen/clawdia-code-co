...
<dict>
    ...
    <dict>
        <key>PATH</key>
        <string>${HOME}/.local/bin:${HOME}/.bun/bin:/usr/local/bin:/usr/bin:/bin</string>
        <key>HOME</key>
        <string>${HOME}</string>
    </dict>
${opts.keepAlive ? `
...
const SERVICES: Record<string, ServiceConfig> = {
  ...
  reflection: {
    label: "com.claude.weekly-reflection",
    script: "examples/weekly-reflection.ts",
    keepAlive: false,
    calendarIntervals: [{ Hour: 8, Minute: 0, Weekday: 0 }], // Sunday 8 AM
    description: "Weekly reflection (Sundays at 8 AM)",
  },
  sync: {
    label: "com.claude.session-sync",
    script: "src/sync-claude-sessions.ts",
    keepAlive: false,
