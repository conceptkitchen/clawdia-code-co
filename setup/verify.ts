...
async function main() {
  ...
  // 5. Tool integrations
  console.log(`\n${bold("  Tool Integrations")}`);

  // GitHub
  env.GITHUB_PAT && !env.GITHUB_PAT.includes("your_")
    ? pass("GitHub PAT configured")
    : warn("No GITHUB_PAT — GitHub MCP won't work");

  // Vercel
  env.VERCEL_API_TOKEN && !env.VERCEL_API_TOKEN.includes("your_")
    ? pass("Vercel API token configured")
    : warn("No VERCEL_API_TOKEN — Vercel MCP won't work");

  // Oura
  env.OURA_API_TOKEN && !env.OURA_API_TOKEN.includes("your_")
    ? pass("Oura API token configured")
    : warn("No OURA_API_TOKEN — health data won't be available");

  // icalBuddy
  existsSync("/opt/homebrew/bin/icalBuddy")
    ? pass("icalBuddy found at /opt/homebrew/bin/icalBuddy")
    : warn("icalBuddy not found — calendar events won't work (brew install ical-buddy)");

  // MCP config
  existsSync(join(PROJECT_ROOT, ".mcp.json"))
    ? pass("MCP config (.mcp.json) exists")
    : warn("No .mcp.json — browser/Vercel MCP won't work in relay subprocess");

  // SDK integration
  existsSync(join(PROJECT_ROOT, "node_modules", "@anthropic-ai", "claude-agent-sdk"))
    ? pass("Claude Agent SDK installed")
    : fail("Claude Agent SDK missing — run: bun add @anthropic-ai/claude-agent-sdk");

  // Memory directory
  const memoryDir = join(homedir(), ".claude-relay", "memory");
  existsSync(memoryDir)
    ? pass(`Memory directory: ${memoryDir}`)
    : warn("Memory directory not created yet (will be created on first message)");

  // State file
  const stateFile = join(homedir(), ".claude-relay", "heartbeat-state.json");
  existsSync(stateFile)
    ? pass("Heartbeat state file exists")
    : warn("Heartbeat state not initialized yet (will be created on first check-in)");

  // 6. Optional
  console.log(`\n${bold("  Optional")}`);
  env.VOICE_PROVIDER
    ? pass(`Voice provider: ${env.VOICE_PROVIDER}`)
    : warn("No VOICE_PROVIDER — voice messages won't be transcribed");
